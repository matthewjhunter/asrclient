...
// TestWithTLSInsecureSkipVerify_KeepsTransportSettings guards the
// second half of the same defect: the option used to replace the
// transport with a bare &http.Transport{}, silently discarding proxy
// support, dial and handshake timeouts, and HTTP/2. It should set the
// TLS flag and leave everything else alone.
func TestWithTLSInsecureSkipVerify_KeepsTransportSettings(t *testing.T) {
	c := NewClient("", WithTLSInsecureSkipVerify())
	...
	if tr.Proxy == nil {
		t.Error("Proxy handling was dropped")
	}
	if tr.TLSHandshakeTimeout == 0 {
		t.Error("TLSHandshakeTimeout was dropped")
	}
	if !tr.ForceAttemptHTTP2 {
		t.Error("HTTP/2 upgrade was dropped")
	}
}

// TestWithHTTPClient_TransportUntouched: a caller who supplies their
// own client owns its transport. The library must not swap it.
func TestWithHTTPClient_TransportUntouched(t *testing.T) {
	own := &http.Transport{MaxIdleConns: 7}
	c := NewClient("", WithHTTPClient(&http.Client{Transport: own}))

	if c.hc.Transport != own {
		t.Error("WithHTTPClient's transport was replaced")
	}
	if err := c.Close(); err != nil {
